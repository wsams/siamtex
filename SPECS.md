...
| F-41b | Users can **import Word `.docx`** via Add file: text is extracted from the OOXML package without executing macros; optional AI conversion proposes LaTeX files with **review-before-accept**; size limits apply (see env `SIAMTEX_MAX_DOCX_*`). |
